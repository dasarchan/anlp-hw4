            ...
            for bug_type in bug_data[lang]:
                ...
                if not os.path.exists(save_dir):
                    ...
                    for case in tqdm.tqdm(bug_data_split, desc=f"{lang}_{bug_type}"):
                        fixed_code, fixing_exp = debugger.debug(
                            lang=lang, code=case["buggy_code"]
                        )
                        print("fixed_code is ", fixed_code)
                        print("task_id is ", case["slug"])
                        rw, res_dict = tester.test(
                            code=fixed_code, language=lang, task_id=case["slug"]
                        )
