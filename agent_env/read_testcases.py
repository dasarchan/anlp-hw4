...
with open("evaluation/all_problems_with_cases.json", "r") as f:
    json_data = json.load(f)

print(len(json_data))
def extract_bug_type(filename):
    """Extract the bug type from a filename of format 'language_bugtype.json'"""
    if '_' in filename and '.json' in filename:
        return filename.split('_')[1].split('.json')[0]
    return None
...
def main():
    ...
    for i in tqdm(sampled_indices, desc="Processing examples"):
        ...
        code = json_data[i]["buggy_code"]
        problem_description = json_data[i]["description"]
        ...
        try:
            fixed_code, num_turns, conversation = execute_debug_agent(
                code, problem_description, openai_handler
            )
